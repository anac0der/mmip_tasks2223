...
def save_mask(fname, snake, img):
    ...
    skio.imsave(fname, blue.astype('uint8'))
    plt.ion()
    
def display_snake(img, init_snake, result_snake, save=False, save_path=None):
    fig, ax = display_image_in_actual_size(img)
...
def iou(true, pred):
    pred[pred > 0] = 1
    true[true > 0] = 1
    overlap = pred * true  # Logical AND
    union = (pred + true) > 0  # Logical OR
